<h1 align="center">Phan Thi Xuan Tien</h1>

<p align="center">
  <b>Data / AI Engineering Student @ UIT VNU-HCM</b><br/>
  Building data pipelines, lakehouse workflows, and applied AI systems
</p>

<p align="center">
  <a href="mailto:xuantienphan2005@gmail.com">
    <img src="https://img.shields.io/badge/Email-xuantienphan2005%40gmail.com-0d1117?style=flat-square&logo=gmail&logoColor=EA4335&labelColor=161b22"/>
  </a>
  <a href="www.linkedin.com/in/selinaphan">
    <img src="https://img.shields.io/badge/LinkedIn-Connect-0d1117?style=flat-square&logo=linkedin&logoColor=0A66C2&labelColor=161b22"/>
  </a>
  <a href="https://github.com/SelinaPhan0205">
    <img src="https://img.shields.io/badge/GitHub-SelinaPhan0205-0d1117?style=flat-square&logo=github&logoColor=white&labelColor=161b22"/>
  </a>
</p>
---

## About

Information Technology undergraduate at the University of Information Technology (UIT), VNU-HCM — GPA **9.0/10.0**, expected graduation **March 2027**.

I am interested in **data engineering**, **real-time analytics**, and **applied AI systems**. My work focuses on building practical data workflows, from data ingestion and lakehouse processing to forecasting, retrieval workflows, and AI-powered applications.

Currently targeting **Data Engineer Intern**, **AI Engineer Intern**, and applied **ML/AI Engineering** opportunities.

- 🎓 **UIT VNU-HCM** — Information Technology, GPA **9.0/10.0**
- 🏅 Academic merit scholarship for **5 consecutive semesters**
- 🌐 **IELTS Academic** — 6.5/9.0
- 📜 NVIDIA Certificate — Applications of AI for Anomaly Detection
- 📍 Ho Chi Minh City, Vietnam

---

## Featured Projects

### 🔴 [TrustFlow — Real-Time KOL Analytics Platform](https://github.com/SelinaPhan0205/TrustFlow)
> Real-time KOL trustworthiness and campaign analytics system using streaming data and lakehouse architecture

**Role:** Streaming Data Engineer / Main Contributor

- Developed the streaming data branch for real-time KOL and campaign analytics
- Built event ingestion and Spark Structured Streaming processing with Kafka/Redpanda
- Integrated streaming outputs into the lakehouse workflow across MinIO, Iceberg, Trino, Airflow, and Docker
- Contributed to a research paper on real-time Lambda Architecture for multi-dimensional KOL trustworthiness evaluation

`Python` `Spark Structured Streaming` `Kafka/Redpanda` `MinIO` `Iceberg` `Trino` `Airflow` `Docker`

---

### 📊 [SME Pulse — Financial Data Processing System](https://github.com/SelinaPhan0205/SME_Pulse)
> Financial analytics system for SME data processing, revenue forecasting, and payment decision support

**Role:** Project Lead

- Led data collection, preprocessing, and lakehouse-oriented data organization
- Designed heuristic-based invoice and payment prioritization logic
- Supported Prophet-based revenue forecasting workflow
- Designed web workflow, UI flow, and API interaction flow for the application

`Python` `SQL` `Lakehouse` `Prophet` `Airflow` `PostgreSQL` `Figma`

---

### ⚖️ [VietContract Auditor — Vietnamese Legal Contract Auditing](https://github.com/SelinaPhan0205/Viet-Contract-Auditor)
> AI-assisted legal contract auditing system using retrieval and multi-agent workflow design

**Role:** AI Workflow Contributor

- Contributed to the design of a LangGraph-based multi-agent workflow for legal contract auditing
- Designed LightRAG-based retrieval and critic-review flow to support legal evidence grounding
- Worked on the audit workflow from contract input to finding generation and report output

`Python` `LangGraph` `LightRAG` `RAG` `Multi-Agent Workflow` `NLP`

---

## Research

**TRUSTFLOW: A Real-Time Lambda Architecture for Multi-Dimensional KOL Trustworthiness Evaluation on Anti-Bot Social Platforms**  
Nguyen Anh Tuan, **Phan Thi Xuan Tien**, Pham Nguyen Phuc Toan, and Ha Minh Tan  
*Manuscript under submission / awaiting review result*

---

## Tech Stack

**Data Engineering**

![Apache Spark](https://img.shields.io/badge/Spark-E25A1C?style=flat-square&logo=apachespark&logoColor=white)
![Apache Kafka](https://img.shields.io/badge/Kafka-231F20?style=flat-square&logo=apachekafka&logoColor=white)
![Apache Airflow](https://img.shields.io/badge/Airflow-017CEE?style=flat-square&logo=apacheairflow&logoColor=white)
![dbt](https://img.shields.io/badge/dbt-FF694B?style=flat-square&logo=dbt&logoColor=white)
![Apache Iceberg](https://img.shields.io/badge/Iceberg-2D2D2D?style=flat-square&logo=apache&logoColor=white)
![Trino](https://img.shields.io/badge/Trino-DD00A1?style=flat-square&logo=trino&logoColor=white)

**AI / ML Systems**

![MLflow](https://img.shields.io/badge/MLflow-0194E2?style=flat-square&logo=mlflow&logoColor=white)
![Prophet](https://img.shields.io/badge/Prophet-0d1117?style=flat-square&logo=meta&logoColor=white)
![LangGraph](https://img.shields.io/badge/LangGraph-0d1117?style=flat-square&logo=langchain&logoColor=white)
![LightRAG](https://img.shields.io/badge/LightRAG-0d1117?style=flat-square&logo=openai&logoColor=white)
![Anomaly Detection](https://img.shields.io/badge/Anomaly%20Detection-6f42c1?style=flat-square)
![RAG](https://img.shields.io/badge/RAG%20Workflow-0d1117?style=flat-square)

**Infrastructure & Tools**

![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![MinIO](https://img.shields.io/badge/MinIO-C72E49?style=flat-square&logo=minio&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white)
![Google Colab](https://img.shields.io/badge/Google%20Colab-F9AB00?style=flat-square&logo=googlecolab&logoColor=white)
![Linux](https://img.shields.io/badge/Linux/WSL-FCC624?style=flat-square&logo=linux&logoColor=black)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)

**Languages**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat-square&logo=mysql&logoColor=white)

---

## GitHub Stats

<p align="center">
  <img height="160" src="https://github-readme-stats.vercel.app/api?username=SelinaPhan0205&show_icons=true&theme=github_dark&hide_border=true&count_private=true&hide_title=true&rank_icon=github"/>
  <img height="160" src="https://github-readme-stats.vercel.app/api/top-langs/?username=SelinaPhan0205&layout=compact&theme=github_dark&hide_border=true&langs_count=6"/>
</p>

---

<p align="center">
  <sub>Open to Data Engineer Intern / AI Engineer Intern opportunities · Ho Chi Minh City · 2026</sub>
</p>
